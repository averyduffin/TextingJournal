...
angular.module('scroll-animate-directive', [])
    .controller('aniDistances', ['$scope', '$interval',
	...
	.directive('aniScrollDelay', function($window) {
        return {
            restrict: 'A',
            controller: 'aniDistances',
            transclude: true,
            ...
            link: function(scope, element, attrs) {
                ...
                angular.element($window).bind('scroll', function() {
                    var targetOffset = attrs.aniScrollDelay;
					var edelay = attrs.aniScrollDelayTime;
                    var offset = scope.getScrollOffsets($window);
                    if (offset.y >= targetOffset) {
						scope.delay(edelay)
                    } else {
